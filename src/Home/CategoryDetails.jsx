...
const CategoryDetails = () => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['category', category, currentPage, itemsPerPage, search, sortOrder],
        queryFn: async () => {
            try {
                const { data } = await axiosNormal.get(`/products/${category}?page=${currentPage}&size=${itemsPerPage}&search=${search}&sort=${sortOrder}`);
                return data;
            } catch (error) {
